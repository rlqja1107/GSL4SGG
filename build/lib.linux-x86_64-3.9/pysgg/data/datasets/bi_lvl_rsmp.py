...
def resampling_dict_generation(dataset, category_list, logger):
    ...
    if repeat_dict_dir is not None and repeat_dict_dir != "" or os.path.exists(curr_dir_repeat_dict):
        ...
        for i in range(len(dataset)):
            ...
            tgt_head_idxs = tgt_pair_idxs[:, 0].contiguous().view(-1) # tgt_head_idx는 bounding box 몇 번째인지를 나타냄.
            tgt_tail_idxs = tgt_pair_idxs[:, 1].contiguous().view(-1)

            tgt_sub_labels = [anno.extra_fields['labels'][i].item() for i in tgt_head_idxs]
            tgt_tail_labels = [anno.extra_fields['labels'][i].item() for i in tgt_tail_idxs]
        ...
        F_c /= (total + 1e-11)
            ...
            # F_per_category[k] *= F_marginal[obj2rtype[k]] # Joint Distribution
        rc_cls = {
...
def apply_resampling(index: int, relation: np.ndarray,
                     repeat_dict: Dict, drop_rate, cat_dict, gt_label):
    """
    Args:
        index:
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.DATA_RESAMPLING_METHOD == 'mylvl':
        n_ntypes = int(np.sqrt(cfg.MODEL.ROI_RELATION_HEAD.RGCN.NUM_RELATION))
    ...
    if not consider_type:
        ...
        if r_c > 1:
            ...
            for i, each_rel in enumerate(relation):
                ...
                h_type = cat_dict['labelidx_to_catidx'][str(gt_label[each_rel[0]])]
                t_type = cat_dict['labelidx_to_catidx'][str(gt_label[each_rel[1]])]
                cat_type_list.append((h_type, t_type))
                if rc_cls[(h_type, t_type)].get(rel_label) is not None:
                    selected_rel_idx.append(i)
            # decrease the head classes of repeated image and non-repeated image
            # if the images are repeated, the total times are > 1, then we calculate the decrease time
            # according to the repeat times.
            ...
            if len(selected_rel_idx) > 0:
                ...
                rel_repeat_time = np.array([rc_cls[type][rel] for rel, type in zip(relation[:, -1], cat_type_list)])
                drop_rate_ = (1 - (rel_repeat_time / (total_repeat_times + 1e-11) )) * drop_rate
