...
class FrequencyBias(nn.Module):
    ...
    def __init__(self, cfg, statistics, eps=1e-3):
        ...
        if cfg.BG_FILTER:
            fg_ratio = statistics['fg_matrix'][:,:,1:].sum(-1) / (statistics['fg_matrix'][:,:,:].sum(-1) + statistics['reporting_fg_matrix'])
        else:
            fg_ratio = statistics['fg_matrix'][:,:,1:].sum(-1) / (statistics['fg_matrix'][:,:,:].sum(-1))
            
        self.weak_relation = {(i,j): 0 for i in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES) for j in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES)}
        self.map_obj_class = {}
        for i, obj_cls in enumerate(statistics['obj_classes']):
            self.map_obj_class[i] = obj_cls
            
        # if cfg.FILL_CRITERIA == 'object':
        #     if not cfg.REVERSE:
        #         if cfg.REPORTING_THRES > 0:  
        #             max_ratio = torch.max(fg_ratio).item() if cfg.REPORTING_CRITERIA == 'max' else 1
        #             relation = torch.nonzero(fg_ratio / max_ratio  > cfg.REPORTING_THRES)
        #             for sub, obj in relation:
        #                 self.weak_relation[(sub.item(), obj.item())] = 1
        #         else:
        #             if not cfg.IS_SHIFT:
        #                 max_ratio = torch.max(fg_ratio) if cfg.REPORTING_CRITERIA == 'max' else 1
        #                 relation = fg_ratio / max_ratio
        #                 for i in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES):
        #                     for j in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES):
        #                         self.weak_relation[(i,j)] = relation[i,j].item()
        #             else:
        #                 # Density
        #                 ratio = cfg.DIVIDE_RATIO
        #                 relation = fg_ratio / ratio
        #                 relation = torch.clip(relation, 0, 1)
        #                 for i in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES):
        #                     for j in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES):
        #                         self.weak_relation[(i,j)] = relation[i,j].item()
        #                 print(f"Density: {((statistics['fg_matrix'][:,:,0] * relation).sum() + statistics['fg_matrix'][:,:,1:].sum()) / statistics['fg_matrix'].sum()}%")
                        

        #     else:
        #         only_bg_rel = (statistics['fg_matrix'][:,:,0] > 1) & (statistics['fg_matrix'][:,:,1:].sum(-1) == 0)
        #         relation = torch.nonzero(only_bg_rel)

        #         for sub, obj in relation:
        #             self.weak_relation[(sub.item(), obj.item())] = 1
                    
        # elif cfg.FILL_CRITERIA == 'uniform':
        #     ratio = fg_ratio.clone()
        #     ratio[1:,1:] = 0.037
        #     for i in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES):
        #         for j in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES):
        #             self.weak_relation[(i,j)] = ratio[i,j].item()
                    
        # elif cfg.FILL_CRITERIA == "reverse":
        #     reverse_ratio = fg_ratio.clone()
        #     non_zero_idx = reverse_ratio != 0.0
        #     reverse_ratio[non_zero_idx] = 1- reverse_ratio[non_zero_idx] 
        #     reverse_ratio[reverse_ratio < cfg.BELOW_ZERO] = 0.0
        #     reverse_ratio = reverse_ratio * 0.047
        #     for i in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES):
        #         for j in range(cfg.MODEL.ROI_BOX_HEAD.NUM_CLASSES):
        #             self.weak_relation[(i,j)] = reverse_ratio[i,j].item()

        # elif cfg.FILL_CRITERIA == "zero":
        #     only_bg_rel = (statistics['fg_matrix'][:,:,0] > 1) & (statistics['fg_matrix'][:,:,1:].sum(-1) == 0)
        #     relation = torch.nonzero(only_bg_rel)

        #     for sub, obj in relation:
        #         self.weak_relation[(sub.item(), obj.item())] = cfg.FOR_ZERO_CONSTANT
        #     print(f"Density: {((statistics['fg_matrix'][only_bg_rel][:,0]* cfg.FOR_ZERO_CONSTANT).sum() + statistics['fg_matrix'][:,:,1:].sum()) / statistics['fg_matrix'].sum()}")
            
        # elif cfg.FILL_CRITERIA == "iou_filter":
        #     relation = fg_ratio * cfg.IOU_MULTI
        ...
        # else:
        #     fill_prop = cfg.FILL_BG_PROP
        #     if not cfg.REVERSE:
        #         sorting_by_flatten = fg_ratio.flatten().argsort(descending=True)
        #         n_total_rel = statistics['fg_matrix'][1:,1:,:].sum().item()
        #         n_cur_rel = 0
        #         n_sub_obj = 0
        #         for i in sorting_by_flatten:
        #             ith = int(i.item() / len(statistics['obj_classes'])); jth = i.item() % len(statistics['obj_classes'])
        #             n_rel = statistics['fg_matrix'][ith, jth, 0].item()
        #             n_cur_rel += n_rel
        #             if n_cur_rel / n_total_rel > fill_prop:
        #                 break
        #             self.weak_relation[(ith, jth)] = 1
        #             n_sub_obj += 1
        #     else:
        #         only_bg_rel = (statistics['fg_matrix'][:,:,0] > 1) & (statistics['fg_matrix'][:,:,1:].sum(-1) == 0)
        #         bg_rel_idx = torch.nonzero(only_bg_rel)
        #         random_idx = np.random.permutation(np.arange(len(bg_rel_idx)))
        #         n_total_rel = statistics['fg_matrix'][1:,1:,:].sum().item()
        #         n_cur_rel = 0
        #         n_sub_obj = 0
        #         for i in random_idx:
        #             ith = bg_rel_idx[i, 0].item(); jth = bg_rel_idx[i, 1].item()
        #             n_rel = statistics['fg_matrix'][ith, jth, 0].item()
        #             n_cur_rel += n_rel
        #             if n_cur_rel / n_total_rel > fill_prop:
        #                 break
        #             self.weak_relation[(ith, jth)] = 1
        #             n_sub_obj += 1
        #     print(f"The number of connected pair: {n_sub_obj}, Ratio: {n_sub_obj / ((len(statistics['obj_classes'])-1) * (len(statistics['obj_classes'])-1)):.3f}")
                    
                    
        # for i, j in zip(torch.where(statistics['fg_matrix'][:,:,0] > 3000)[0][:10], torch.where(statistics['fg_matrix'][:,:,0] > 3000)[1][:10]):
        #     sub = obj_classes[i]; obj = obj_classes[j]
        #     classes = statistics['fg_matrix'][i,j]
        #     bg = statistics['fg_matrix'][i,j][0]
        #     fg = statistics['fg_matrix'][i,j,1:].sum()
        #     ratio = fg / (bg+fg)
        #     print(f"sub:{sub}, obj:{obj}, bg:{bg}, fg:{fg}, ratio: {ratio}")
