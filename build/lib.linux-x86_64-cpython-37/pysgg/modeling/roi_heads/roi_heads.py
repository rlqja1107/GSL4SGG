...
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def __init__(self, cfg, heads):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR in ['Hetero_RGCN', 'ARGCN_Predictor', 'HetSGG_Predictor', 'HetSGG_Predictor_1', 'HetSGG_Predictor_2', 'HetSGG_Predictor_NS']:
    ...
    def compute_category(self, predictions):
        if self.mode != 'predcls':
            if self.cfg.MODEL.ROI_RELATION_HEAD.RGCN.CLASS_AGG == 'sum' or self.cfg.MODEL.ROI_RELATION_HEAD.RGCN.CLASS_AGG == 'max':
                for i, _ in enumerate(predictions):
                    ...
                    if int(self.cfg.MODEL.ROI_RELATION_HEAD.RGCN.CATEGORY_FILE[-1]) in [1, 3]:
                        landform_score = torch.softmax(predictions[i].extra_fields['predict_logits'], 1)[:,self.vg_cat_dict['catidx_labelgroup'][str(3)]].sum(1).view(-1,1)
                        predictions[i].extra_fields['category_scores'] = torch.cat([product_score, human_score, animal_score, landform_score], dim = 1)
                    elif int(self.cfg.MODEL.ROI_RELATION_HEAD.RGCN.CATEGORY_FILE[-1]) in [4]:
                        landform_score = torch.softmax(predictions[i].extra_fields['predict_logits'], 1)[:,self.vg_cat_dict['catidx_labelgroup'][str(3)]].sum(1).view(-1,1)
                        body_score = torch.softmax(predictions[i].extra_fields['predict_logits'], 1)[:,self.vg_cat_dict['catidx_labelgroup'][str(4)]].sum(1).view(-1,1)
                        predictions[i].extra_fields['category_scores'] = torch.cat([product_score, human_score, animal_score, landform_score, body_score], dim = 1)
                    else:
                        predictions[i].extra_fields['category_scores'] = torch.cat([product_score, human_score, animal_score], dim = 1)
                    
                    
            elif self.cfg.MODEL.ROI_RELATION_HEAD.RGCN.CLASS_AGG== 'mean':
                for i, _ in enumerate(predictions):
                    # Revision
                    prod_except_background = self.vg_cat_dict['catidx_labelgroup'][str(0)][:-1]
                    ...
                    if int(self.cfg.MODEL.ROI_RELATION_HEAD.RGCN.CATEGORY_FILE[-1]) in [1, 3]:
                        landform_score = torch.softmax(predictions[i].extra_fields['predict_logits'], 1)[:,self.vg_cat_dict['catidx_labelgroup'][str(3)]].mean(1).view(-1,1)
                        predictions[i].extra_fields['category_scores'] = torch.cat([product_score, human_score, animal_score, landform_score], dim = 1)
                    elif int(self.cfg.MODEL.ROI_RELATION_HEAD.RGCN.CATEGORY_FILE[-1]) in [4]:
                        landform_score = torch.softmax(predictions[i].extra_fields['predict_logits'], 1)[:,self.vg_cat_dict['catidx_labelgroup'][str(3)]].mean(1).view(-1,1)
                        body_score = torch.softmax(predictions[i].extra_fields['predict_logits'], 1)[:,self.vg_cat_dict['catidx_labelgroup'][str(4)]].mean(1).view(-1,1)
                        predictions[i].extra_fields['category_scores'] = torch.cat([product_score, human_score, animal_score, landform_score, body_score], dim = 1)
                    else:
                        predictions[i].extra_fields['category_scores'] = torch.cat([product_score, human_score, animal_score], dim = 1)
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR in ['Hetero_RGCN', 'ARGCN_Predictor']:
