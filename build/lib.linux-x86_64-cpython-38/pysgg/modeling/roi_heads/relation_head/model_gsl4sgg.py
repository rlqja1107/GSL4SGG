...
class GSL4SGG(nn.Module):
    def __init__(
        ...
        self.l2r = nn.Sequential(
            nn.Linear(3*self.pooling_dim, self.pooling_dim),
            nn.ReLU(),
            nn.Dropout(0.5),
                ...
                for idx, each_img_relness in enumerate(rel_prop_pairs_relness_batch):
                    ...
                    if self.apply_gt_for_rel_conf: # False
                        ...
                        if cfg.CONSIDERATION_NON_GT:
                            ...
                            selected_rel_prop_pairs_idx = torch.cat([selected_rel_prop_pairs_idx[gt_rel_idx]])
                    else:
    ...
    def forward(         
        ...
        centroids = []
        centroid = augment_obj_feat.split(num_obj)
        centroid = [torch.mean(c.view(1, -1, self.pooling_dim), dim=1).repeat(len(c),1) for c in centroid]
        rel_inds_batch_cat = []
        offset = 0
        for idx, (prop, rel_ind_i) in enumerate(zip(proposals, rel_pair_inds)):
            ...
            rel_inds_batch_cat.append(rel_ind_i)
        rel_inds_batch_cat = torch.cat(rel_inds_batch_cat, 0)
        residual_node_features = augment_obj_feat - cat(centroid)
        ...
        sim_vec = torch.cat((torch.abs(augment_obj_feat[rel_inds_batch_cat[:,0]] - augment_obj_feat[rel_inds_batch_cat[:,1]]), residual_node_sub, residual_node_obj), dim = 1)
        prob_score = self.l2r(sim_vec)
        pre_adj = torch.sigmoid(prob_score)
        sampled_edge_gt = torch.ones([pre_adj.size(0)]).cuda()
        sampled_edge = pyro.distributions.RelaxedBernoulliStraightThrough(temperature=0.6, probs = pre_adj).rsample() # Bernouli Approximation (Gumbel Softmax)
        combine_weight = self.edge_skip_alpha * (sampled_edge * sampled_edge_gt) + (1-self.edge_skip_alpha) * (sampled_edge * pre_adj)
        ...
        (
            ...
            proposals, rel_pair_inds, relatedness_each_iters[-1] # Relatedness가 1인 곳만 관계 형성
        )
