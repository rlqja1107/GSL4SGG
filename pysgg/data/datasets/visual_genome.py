...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        self.split = split
        self.img_dir = "/home/public/Datasets/CV/vg_bm/VG_100k"
        self.dict_file = "/home/public/Datasets/CV/vg_bm/VG-SGG-dicts-with-attri.json"
        self.roidb_file = "/home/public/Datasets/CV/vg_bm/VG-SGG-with-attri.h5"
        self.image_file = "/home/public/Datasets/CV/vg_bm/image_data.json"
        ...
        # if 'vg' in cfg.DATA_DIR:
        #     self.vg_cat_dict = json.load(open(cfg.DATA_DIR+'/VG-SGG-Category_v2.json', 'r'))
        # else:
        # self.vg_cat_dict = json.load(open(f'/home/public/Datasets/CV/vg_bm/VG-SGG-Category_v2.json', 'r'))
    ...
    def get_statistics(self):
        fg_matrix, bg_matrix, rel_counter_init, reporting_bg_matrix = get_VG_statistics(self,
                                                 must_overlap=True)
        eps = 1e-3
        bg_matrix += 1
        reporting_bg_matrix += 1
        ...
        result = {
            ...
            'reporting_fg_matrix': torch.from_numpy(reporting_bg_matrix)
        }
...
def get_VG_statistics(train_data, must_overlap=True):
    ...
    reporting_bg_matrix = np.zeros((num_obj_classes, num_obj_classes), dtype=np.int64)
    ...
    for ex_ind in tqdm(range(len(train_data.img_info))):
        ...
        relative_total = np.array(
            box_filter(gt_boxes, must_overlap=must_overlap), dtype=int)
        o1o2_total = gt_classes[relative_total]
        for (o1, o2), (ro1, ro2) in zip(o1o2_total, relative_total):
            if cfg.BG_FILTER:
                box1 = torch.FloatTensor(gt_boxes[ro1]).unsqueeze(0); box2 = torch.FloatTensor(gt_boxes[ro2]).unsqueeze(0) # Subject Box, Object Box
                ...
                if iou.item() > 0.5:
                    reporting_bg_matrix[o1, o2] += 1
                    
            bg_matrix[o1, o2] += 1

    return fg_matrix, bg_matrix, rel_counter, reporting_bg_matrix
