...
from pysgg.modeling.roi_heads.relation_head.model_kern import to_onehot
    ...
    def _get_map_idxs(self, proposals, rel_pair_inds, is_training):
        ...
        for proposal, rel_pair_ind in zip(proposals, rel_pair_inds):
            ...
            if self.mode == 'sgcls':
                ...
                if 'labels' in proposal.extra_fields.keys() and is_training:
                    proposal_category_np = proposal_category.cpu().numpy().reshape(-1,1)
                    gt_category_np = self.vg_map_arr[proposal.extra_fields['labels'].long(), 1].cpu().numpy().reshape(-1,1) # GT Category
                    self.gt2pred.append(np.concatenate([gt_category_np, proposal_category_np], -1))
                ...
                proposal_category = proposal.extra_fields['category_scores'].max(1)[1].detach()
                if 'labels' in proposal.extra_fields.keys() and is_training:
                    fg_idx = torch.nonzero(proposal.extra_fields['labels']!=0).squeeze()
                    proposal_category_np = proposal_category[fg_idx].cpu().numpy().reshape(-1,1)
                    faster_pred_label = proposal.extra_fields['pred_labels'][fg_idx].cpu().numpy().reshape(-1,1)
                    faster_pred_cat = self.vg_map_arr[faster_pred_label, 1].cpu().numpy()
                    gt_category_np = self.vg_map_arr[proposal.extra_fields['labels'].long(), 1][fg_idx].cpu().numpy().reshape(-1,1) # GT Category
                    self.gt2pred.append(np.concatenate([gt_category_np, proposal_category_np, faster_pred_cat], -1))
            else:
...
class RGCNLayer(MessagePassing):
    ...
    def __init__(self, node_in_channels, edge_in_channels, node_out_channels, edge_out_channels, num_bases, num_relations, cfg, bias=False):
        ...
        self.entity2rel_attn = nn.ModuleList([nn.Linear(edge_out_channels, 1, bias = False) for _ in range(self.num_relations)])
    ...
    def entity2rel_msg_attn(self, rel, edge_type_rel, edge_type_rel_inv, sub2rel_msg, obj2rel_msg):
        """
        Attention Mechanism from entity to predicate
        """
        sub_type = self.rtype2obj[rel][0]; obj_type = self.rtype2obj[rel][1]
        sub2obj_edge_type = self.obj2rtype[(sub_type, obj_type)]; obj2sub_edge_type = self.obj2rtype[(obj_type, sub_type)]

        category_type = ((edge_type_rel == sub2obj_edge_type) & (edge_type_rel_inv == obj2sub_edge_type)).int()
        # sub2rel_msg_mask = sub2rel_msg * category_type.view(-1,1)
        # obj2rel_msg_mask = obj2rel_msg * category_type.view(-1,1)
        sub2rel_score = self.entity2rel_attn[rel](sub2rel_msg)
        obj2rel_score = self.entity2rel_attn[rel](obj2rel_msg)

        mask_idx = (category_type==0).view(-1)
        entity_cat_msg = F.leaky_relu(torch.cat([sub2rel_score, obj2rel_score], 1))
        entity_score = torch.softmax(entity_cat_msg, 1)
        # entity_score[mask_idx.view(-1), :] = 0.0
    ...
    def propagate(self, edge_index, nf, ef, edge_type_list, edge_norm_list, size=None):
        ...
        # edge_norm_rel, edge_norm_rel_inv = edge_norm_list
        ...
        entity_msg = []
        for rel in range(self.num_relations):
            msg = self.entity2rel_msg_attn(rel, edge_type_rel, edge_type_rel_inv, sub2rel_msg, obj2rel_msg)
            entity_msg.append(msg[..., None])

        entity_msg = torch.cat(entity_msg, -1)
        rel_embedding = x_ij + torch.sum(entity_msg, -1)
        
        # sub_msg = self.message(edge_index[0, :], x_i, None, rel_embedding, edge_type_rel_inv, 'rel2sub')* edge_norm_rel[..., None, None]
        # obj_msg = self.message(edge_index[1, :], x_j, None, rel_embedding, edge_type_rel, 'rel2obj')* edge_norm_rel_inv[..., None, None]
        ...
        #Option 1
        # relational noramlize msg 
        if self.use_attn_agg:
            sub_agg, _ = self.rel2sub_salayer(sub_agg, sub_agg, sub_agg, node_mask_sub_gt)
            obj_agg, _ = self.rel2obj_salayer(obj_agg, obj_agg, obj_agg, node_mask_obj_gt)
