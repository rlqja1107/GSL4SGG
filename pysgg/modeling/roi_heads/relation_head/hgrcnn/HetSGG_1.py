...
class HetSGG_1(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        super(HetSGG_1, self).__init__()
...
class RGCNLayer(MessagePassing):
    ...
    def __init__(self, node_in_channels, edge_in_channels, node_out_channels, edge_out_channels, num_bases, num_relations, cfg, bias=False):
        ...
        self.exclude_ablation = cfg.MODEL.ROI_RELATION_HEAD.RGCN.EXCLUDE_ABLATION
        self.obj2rtype = {(i, j): self.n_ntypes*j+i for j in range(self.n_ntypes) for i in range(self.n_ntypes)}
        self.rtype2obj = dict(zip(self.obj2rtype.values(), self.obj2rtype.keys()))

        layers = []
        # Ablation Study
        if 'edge' not in self.exclude_ablation:
            ...
            layers.append(self.sub2rel_basis.data); layers.append(self.sub2rel_att.data)
            layers.append(self.obj2rel_basis.data); layers.append(self.obj2rel_att.data)
        else:
            self.sub2rel_msg = nn.Linear(node_in_channels, edge_out_channels)
            self.obj2rel_msg = nn.Linear(node_in_channels, edge_out_channels)
            layers.append(self.sub2rel_msg.weight.data)
            layers.append(self.obj2rel_msg.weight.data)

        if 'node' not in self.exclude_ablation:
            ...
            layers.append(self.rel2obj_basis.data); layers.append(self.rel2obj_att.data)
            layers.append(self.rel2sub_basis.data); layers.append(self.rel2sub_att.data)
        else:
            self.rel2sub_msg = nn.Linear(edge_in_channels, node_out_channels)
            self.rel2obj_msg = nn.Linear(edge_in_channels, node_out_channels)
            self.rel2sub_attn = nn.Linear(node_out_channels, 1)
            self.rel2obj_attn = nn.Linear(node_out_channels, 1)
            layers.append(self.rel2sub_msg.weight.data); layers.append(self.rel2sub_attn.weight.data)
            layers.append(self.rel2obj_msg.weight.data); layers.append(self.rel2obj_attn.weight.data)
    ...
    def propagate(self, edge_index, nf, ef, edge_type_list, edge_norm_list, size=None):
        ...
        if 'node' not in  self.exclude_ablation:
            ...
            # ones_torch = torch.ones_like(edge_type_rel)[..., None]
            # sub_degree = self.aggregate(ones_torch, index=edge_index[0, :], ptr=None, dim_size=nf.size(0))
            # obj_degree = self.aggregate(ones_torch, index=edge_index[1, :], ptr=None, dim_size=nf.size(0))
            sub_attn_score = F.leaky_relu(self.rel2sub_attn(sub_msg[..., 0]))
            obj_attn_score = F.leaky_relu(self.rel2obj_attn(obj_msg[..., 0]))
            sub_attn_score = self.softmax(sub_attn_score, edge_index[0, :])# ij_r, edgeindex_i
            obj_attn_score = self.softmax(obj_attn_score, edge_index[1, :])
            sub_msg = sub_attn_score * sub_msg.squeeze(2)
            obj_msg = obj_attn_score * obj_msg.squeeze(2)
