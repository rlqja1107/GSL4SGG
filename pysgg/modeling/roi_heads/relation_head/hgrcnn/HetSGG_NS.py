...
class HetSGG_NS(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        super(HetSGG_NS, self).__init__()
        ...
        if self.feat_update_step > 0:
            ...
            self.rgcn = nn.ModuleList([HetSGG_Layer(self.dim, self.dim, self.dim, self.dim, self.num_bases, self.n_reltypes, cfg=cfg) for _ in range(self.feat_update_step)])
        ...
        if self.score_update_step > 0:
            ...
            self.rgcn_score = nn.ModuleList([HetSGG_Layer(num_classes_obj, num_classes_pred, num_classes_obj, num_classes_pred, num_bases=self.num_bases, num_relations=self.n_reltypes, cfg=cfg) for _ in range(self.score_update_step)])
    ...
    def forward(self,      
        ...
        for i in range(self.feat_update_step):
            nf, ef = self.rgcn[i](g_rel, nf, ef, edge_type_list, edge_norm_list)
            nf = F.elu(nf)
            ef = F.elu(ef)
        ...
        # Logit Layer
        for i in range(self.score_update_step):
            obj_class_logits, pred_class_logits = self.rgcn_score[i](g_rel, obj_class_logits, pred_class_logits, edge_type_list, edge_norm_list)
            obj_class_logits = F.elu(obj_class_logits)
            pred_class_logits = F.elu(pred_class_logits)
            pred_logits.append(pred_class_logits)


        return obj_class_logits, pred_class_logits, pred_logits
...
class HetSGG_Layer(MessagePassing):
    ...
    def __init__(self, node_in_channels, edge_in_channels, node_out_channels, edge_out_channels, num_bases, num_relations, cfg, bias=False):
        super(HetSGG_Layer, self).__init__()
        ...
        # self.sub2rel_attn = nn.ModuleList([nn.Linear(edge_out_channels, 1) for _ in range(self.num_relations)])
