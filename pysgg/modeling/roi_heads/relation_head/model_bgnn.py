        ...
        for refine_iter in range(self.mp_pair_refine_iter): # 3
            ...
            if self.rel_aware_on: # True
                ...
                if self.cfg.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.USE_GLOBAL_CONTEXT:
                    if self.cfg.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.USE_OBJ_OR_REL_CONTEXT == 'obj':
                        refine_feats = refine_ent_feats_each_iters[-1]
                    else:
                        refine_feats = refine_rel_feats_each_iters[-1]
                    centroid = torch.mean(refine_feats, 0)
                else:
                    centroid = None
                ...
                if not self.shared_pre_rel_classifier: # False
                    pre_cls_logits, pred_relatedness_scores = self.relation_conf_aware_models[
                        refine_iter
                    ](input_features, proposals, rel_pair_inds, centroid)
                else:
                    pre_cls_logits, pred_relatedness_scores = self.relation_conf_aware_models(
