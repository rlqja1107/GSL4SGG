...
def focal_loss_link_predictor(link_prob, rel_labels, loss, lambda_link):
    ...
    # return loss_val_bin
    return loss_val_bin * lambda_link

def bce_loss_link_predictor(link_prob, rel_labels, loss):

    bin_logits = link_prob[rel_labels != -1]

    selected_labels = rel_labels[rel_labels != -1].long()

    onehot = torch.zeros_like(bin_logits)
    onehot[selected_labels > 0] = 1
    loss_val = loss(bin_logits, onehot)
...
class RelAwareLoss(nn.Module):
    ...
    def forward(self, pred_logit, rel_labels):
        if "focal" in self.pre_clser_loss_type:
            ...
            elif self.predictor_type == "hybrid": # Hybrid
...
class GSL_LOSS(nn.Module):
    def __init__(self, cfg):
        ...
        alpha = cfg.MODEL.ROI_RELATION_HEAD.RELATION_PROPOSAL_MODEL.FOCAL_LOSS_ALPHA # 0.2
        gamma = cfg.MODEL.ROI_RELATION_HEAD.RELATION_PROPOSAL_MODEL.FOCAL_LOSS_GAMMA # 2.0
        self.lambda_link = cfg.MODEL.ROI_RELATION_HEAD.GSL_MODULE.REGULARIZATION_LAMBDA
        self.loss_type =  cfg.MODEL.ROI_RELATION_HEAD.GSL_MODULE.LINK_LOSS_TYPE
    ...
    def forward(self, link_prob, rel_labels):
        if self.loss_type == 'focal':
            return focal_loss_link_predictor(link_prob, rel_labels, self.loss_module, self.lambda_link)
        elif self.loss_type == 'bce':
            return  bce_loss_link_predictor(link_prob, rel_labels, self.loss_module)
        else:
            assert("Assure the link predictor loss type")
