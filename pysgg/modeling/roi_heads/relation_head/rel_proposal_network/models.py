...
def gt_rel_proposal_matching(proposals, targets, fg_thres, require_overlap, weak_relation):
    """
    ...
    for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
        ...
        is_match = ious > fg_thres  # [tgt, prp] # 0.5
        # one box may match multiple gt boxes here we just mark them as a valid matching if they
        # match any boxes
        locating_match = (ious > fg_thres).nonzero(as_tuple=False)  # [tgt, prp]
        ...
        if cfg.IS_REPORTING:
            num_obj = tgt_rel_matrix.shape[0]
            
            weak_tgt_rel = tgt_rel_matrix.clone()
            
            for i in range(num_obj):
                for j in range(num_obj):
                    if i == j: continue
                    sub_idx = int(target.extra_fields["labels"][i].item())
                    obj_idx = int(target.extra_fields['labels'][j].item())
                    prop = np.random.random(1)
                    sub_obj_rel_prob = weak_relation[(sub_idx, obj_idx)]
                    if sub_obj_rel_prob > prop and weak_tgt_rel[i,j] == 0:
                        weak_tgt_rel[i,j] = 1
            weak_tgt_pair_idxs = torch.nonzero(weak_tgt_rel)
            num_weak_tgt_rels = weak_tgt_pair_idxs.shape[0]

            weak_tgt_head_idxs = weak_tgt_pair_idxs[:, 0].contiguous().view(-1)
            weak_tgt_tail_idxs = weak_tgt_pair_idxs[:, 1].contiguous().view(-1)

            binary_prp_head = is_match[weak_tgt_head_idxs]
            # num_tgt_rel, num_prp (matched prp tail)
            binary_prp_tail = is_match[weak_tgt_tail_idxs]
            # mark the box pair who overlaps with the gt relation box pairs
            num_prp = len(proposal)

            for i in range(num_weak_tgt_rels):
                bi_match_head = torch.nonzero(binary_prp_head[i] > 0)
                bi_match_tail = torch.nonzero(binary_prp_tail[i] > 0)
                num_bi_head = bi_match_head.shape[0]
                num_bi_tail = bi_match_tail.shape[0]
            ...
            # for i in range(num_prp):
            #     for j in range(num_prp):
            #         if i == j: continue
            #         sub_idx = int(proposal.extra_fields['labels'][i].item())
            #         obj_idx = int(proposal.extra_fields['labels'][j].item())
            #         prop = np.random.random(1)
            #         sub_obj_rel_prob = weak_relation[(sub_idx, obj_idx)]
            #         if sub_obj_rel_prob > prop and binary_rel_mat[i,j] == 0:
            #             binary_rel_mat[i,j] = 1
...
def gt_gsl_matching_box(proposals, targets, fg_thres, require_overlap, weak_relation):
    """
...
class RelAwareRelFeature(nn.Module):
    def __init__(
        ...
        if self.visual_features_on:
            ...
            if self.cfg.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.USE_GLOBAL_CONTEXT:
                self.global_embed = nn.Sequential(
                    nn.ReLU(),
                    nn.Linear(self.input_dim, self.hidden_dim),
                )
                
            channel = 3 if self.cfg.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.USE_GLOBAL_CONTEXT else 2
    ...
    def forward(
        ...
        if self.visual_features_on: # True
            ...
            if centroid is not None:
                centroid = self.global_embed(centroid.detach())
        visual_feat_split = torch.split(visual_feat, [len(p) for p in rel_pair_inds], dim=0)
            ...
            if self.visual_features_on: # True
                ...
                if centroid is not None:
                    rel_prop_repre = self.proposal_feat_fusion(
                        torch.cat((visual_relness_feat, prop_pair_geo_feat, centroid.repeat(prop_pair_geo_feat.shape[0], 1)), dim=1)
                    )
                else:
                    rel_prop_repre = self.proposal_feat_fusion(
