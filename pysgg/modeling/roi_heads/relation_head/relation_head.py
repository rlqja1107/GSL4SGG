...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.union_feature_extractor = make_roi_relation_feature_extractor( # FPN2MLPFeatureExtractor 이용
            cfg,
            in_channels,
        )
        if cfg.MODEL.ATTRIBUTE_ON:
            self.box_feature_extractor = make_roi_box_feature_extractor( # FPN2MLPFeatureExtractor
        ...
        self.weak_relation = self.predictor.freq_bias.weak_relation
        ...
        self.rel_prop_type = self.cfg.MODEL.ROI_RELATION_HEAD.RELATION_PROPOSAL_MODEL.METHOD # RelAwareRelFeature, rel_pn
        ...
        if self.rel_prop_on:
            if self.rel_prop_type == "rel_pn":
                ...
                self.use_relness_ranking = (
                    cfg.MODEL.ROI_RELATION_HEAD.RELATION_PROPOSAL_MODEL.USE_RELATEDNESS_FOR_PREDICTION_RANKING # False
                )
        ...
        self.gnn_fully_connected = cfg.MODEL.ROI_RELATION_HEAD.GSL_MODULE.TRAIN_ON_FULLY_CONNECTED
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.rel_prop_on: # True
            ...
            if targets is not None and self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR != 'GSL4SGG_Predictor':
                ...
                fg_pair_matrixs, gt_rel_binarys_matrix = gt_gsl_matching_box(
                    proposals,
                    targets,
                    self.cfg.MODEL.ROI_HEADS.FG_IOU_THRESHOLD,
                    self.cfg.TEST.RELATION.REQUIRE_OVERLAP,
            ...
            if self.rel_prop_type == "rel_pn":
                ...
                if self.cfg.MODEL.ROI_RELATION_HEAD.RGCN.USE_REL_PN and not self.training:
                    _, _, selected_rel_pairs_idx = filter_rel_pairs(relness_matrix, rel_pair_idxs, rel_labels)
                elif self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'Purifier_GRCNN' or self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'AGRCNNPredictor':
        ...
        if self.use_union_box: # True
            union_features = self.union_feature_extractor(features, proposals, rel_pair_idxs) # RelationFeatureExtractor => Spaital Information 포함
        else:
            union_features = None
        ...
        if self.gnn_fully_connected:
            rel_pair_idxs = rel_idx_pairs_fully_connected
            rel_pn_labels = gt_rel_label_fully_connected
        
        # Analysis
        gt_relatedness = targets[0].extra_fields['relation']
        # gt_relatedness = None
        ...
        loss_relation, loss_refine = self.loss_evaluator(
            proposals, rel_labels, relation_logits, obj_refine_logits # relation, object Logit으로 Loss 내기 => Refine : Object
        )
