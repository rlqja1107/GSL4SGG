...
from pysgg.modeling.roi_heads.relation_head.model_gsl4sgg import GSL4SGG
from pysgg.modeling.roi_heads.relation_head.model_gsl4sgg_contrast import GSL4SGG_CONTRAST
...
from .model_bgnn_reporting import BGNNContext_Reporting

from .model_agcn import GRCNN

from pysgg.modeling.roi_heads.relation_head.hgrcnn.RGCN_noskip import RGCN_NOSKIP
from pysgg.modeling.roi_heads.relation_head.hgrcnn.HetSGG import HetSGG
from pysgg.modeling.roi_heads.relation_head.hgrcnn.ARGCN import ARGCN
...
@registry.ROI_RELATION_PREDICTOR.register("BGNNPredictor_Reporting")
class BGNNPredictor_Reporting(nn.Module):
    def __init__(self, config, in_channels):
        super(BGNNPredictor_Reporting, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        self.use_bias = config.MODEL.ROI_RELATION_HEAD.FREQUENCY_BAIS
        ...
        if self.split_context_model4inst_rel: # False
            self.obj_context_layer = BGNNContext_Reporting(
                config,
                self.input_dim,
                hidden_dim=self.hidden_dim,
                num_iter=config.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.GRAPH_ITERATION_NUM,
            )
            self.rel_context_layer = BGNNContext_Reporting(
                config,
                self.input_dim,
                hidden_dim=self.hidden_dim,
                num_iter=config.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.GRAPH_ITERATION_NUM,
            ...
            self.context_layer = BGNNContext_Reporting(
                config,
                self.input_dim,
                hidden_dim=self.hidden_dim,
                num_iter=config.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.GRAPH_ITERATION_NUM, # 3 => Graph 이용 여기에 넣으면 된다.
            )
        ...
        self.fg_bg_score = {'fg':{0:{'0~5': [], '6~10':[], '11~15':[], '16~20':[], '21~':[]},
                            1:{'0~5': [], '6~10':[], '11~15':[], '16~20':[], '21~':[]},
                            2:{'0~5': [], '6~10':[], '11~15':[], '16~20':[], '21~':[]}},
                            'bg': {0:{'0~5': [], '6~10':[], '11~15':[], '16~20':[], '21~':[]},
                            1:{'0~5': [], '6~10':[], '11~15':[], '16~20':[], '21~':[]},
                            2:{'0~5': [], '6~10':[], '11~15':[], '16~20':[], '21~':[]}}
                            }
        # for logging things
    ...
    def define_interval(self, n_prop):
        if n_prop >= 0 and n_prop <=5:
            return '0~5'
        elif n_prop >= 6 and n_prop <=10:
            return '6~10'
        elif n_prop >=11 and n_prop <=15:
            return '11~15'
        elif n_prop >=16 and n_prop <=20:
            return '16~20'
        else:
            return '21~'  
    ...
    def forward(
        ...
        obj_feats, rel_feats, pre_cls_logits, relatedness = self.context_layer( # GNN을 사용하는 부분
            roi_features, union_features, inst_proposals, rel_pair_idxs, rel_binarys, logger
        )
        gt_pair = torch.nonzero(gt_relation_matrix)
        n_prop = obj_feats.shape[0]
        all_pair = torch.ones((n_prop, n_prop)) - torch.eye(n_prop)
        all_pair[gt_pair[:,0], gt_pair[:,1]] = 0
        bg_pair = torch.nonzero(all_pair)
        interval = self.define_interval(n_prop)
        for i in range(3):
            fg_score = relatedness[0,:,:,i][gt_pair[:,0], gt_pair[:,1]]
            bg_score = relatedness[0,:,:,i][bg_pair[:,0], bg_pair[:,1]]
            self.fg_bg_score['fg'][i][interval].extend(fg_score.detach().cpu().numpy())
            self.fg_bg_score['bg'][i][interval].extend(bg_score.detach().cpu().numpy())
        ...
        if self.mode == "predcls":
            ...
            refined_obj_logits = self.obj_classifier(obj_feats) # Linear 태움

        rel_cls_logits = self.rel_classifier(rel_feats)  # Linear태움
        ...
        # For correcting the reporting bias - Trainig with GT relation corrected reporting bias

        add_losses = {}
        ## pre clser relpn supervision
        if pre_cls_logits is not None and self.training:
            if cfg.IS_REPORTING:
                rel_weak_labels = cat(rel_labels, dim=0).clone()
                self.rel_weak_labels(inst_proposals, rel_pair_idxs, rel_weak_labels)
            else:
                rel_weak_labels = None
            rel_labels = cat(rel_labels, dim=0)
            if rel_weak_labels is None:
                rel_weak_labels = rel_labels

            for iters, each_iter_logit in enumerate(pre_cls_logits):
                if len(squeeze_tensor(torch.nonzero(rel_labels != -1))) == 0:
                    loss_rel_pre_cls = None
                else:
                    loss_rel_pre_cls = self.rel_aware_loss_eval(each_iter_logit, rel_weak_labels)
    ...
    def rel_weak_labels(self, inst_proposals, rel_pair_idxs, rel_weak_labels):
        idx = -1
        for proposal, rel_pair_idx in zip(inst_proposals, rel_pair_idxs):
            for sub, obj in rel_pair_idx:
                idx += 1
                sub = sub.item(); obj = obj.item()
                # For Upper bound
                # sub_idx = proposal.extra_fields['labels'][sub].item() if self.training else proposal.extra_fields['pred_labels'][sub].item()
                # obj_idx = proposal.extra_fields['labels'][obj].item() if self.training else proposal.extra_fields['pred_labels'][obj].item()
                sub_idx = proposal.extra_fields['labels'][sub].item()
                obj_idx = proposal.extra_fields['labels'][obj].item()
                
                sub_idx = int(sub_idx); obj_idx = int(obj_idx)
                # Bernoulli Sampling
                prop = np.random.random(1)
                sub_obj_rel_prob = self.freq_bias.weak_relation[(sub_idx, obj_idx)]
                if sub_obj_rel_prob > prop and rel_weak_labels[idx] < 1:
                    rel_weak_labels[idx] = 1
...
@registry.ROI_RELATION_PREDICTOR.register("GSL4SGG_Predictor")
class GSL4SGG_Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(GSL4SGG_Predictor, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        self.use_bias = config.MODEL.ROI_RELATION_HEAD.FREQUENCY_BAIS
        ...
        self.pooling_dim = cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM
        self.input_dim = in_channels
        self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.GSL_MODULE.HIDDEN_DIM

        self.context_layer = GSL4SGG(config, in_channels, hidden_dim=self.hidden_dim)
        ...
        self.rel_aware_model_on = config.MODEL.ROI_RELATION_HEAD.GSL_MODULE.USE_LINK_REMOVER # True
    ...
    def forward(
        ...
        obj_feats, rel_feats, pre_adj, residual_weight_list = self.context_layer( # GNN을 사용하는 부분
            roi_features, union_features, inst_proposals, rel_pair_idxs, rel_binarys, logger
        )
        # gt_pair = torch.nonzero(gt_relation_matrix)
        # n_prop = obj_feats.shape[0]
        # all_pair = torch.ones((n_prop, n_prop)) - torch.eye(n_prop)
        # all_pair[gt_pair[:,0], gt_pair[:,1]] = 0
        # bg_pair = torch.nonzero(all_pair).to(obj_feats.device)
        # interval = self.define_interval(n_prop)
        # rel_pair = rel_pair_idxs[0]
        # for i in range(3):
        #     for p in gt_pair:
        #         relative_idx = torch.nonzero((p[None, ...] == rel_pair).all(-1)).item()
        #         fg_score = residual_weight_list[i][relative_idx]
        #         self.fg_bg_score['fg'][i][interval].append(float(f"{fg_score.item():.4f}"))
                
        #     for p in bg_pair:
        #         relative_idx = torch.nonzero((p[None, ...] == rel_pair).all(-1)).item()
        #         bg_score = residual_weight_list[i][relative_idx]
        #         self.fg_bg_score['bg'][i][interval].append(float(f"{bg_score.item():.4f}"))              
        ...
        add_losses = {}
        ## pre clser relpn supervision
        if pre_adj is not None and self.training:
            rel_labels = cat(rel_labels, dim=0)
            for iters, each_iter_link_likelihood in enumerate(pre_adj):
                if len(squeeze_tensor(torch.nonzero(rel_labels != -1))) == 0:
                    loss_rel_pre_cls = None
                else:
                    loss_rel_pre_cls = self.rel_aware_loss_eval(each_iter_link_likelihood, rel_labels)
...
@registry.ROI_RELATION_PREDICTOR.register("GSL4SGG_Contrast_Predictor")
class GSL4SGG_CONTRAST_Predictor(nn.Module):
    def __init__(self, config, in_channels):
        super(GSL4SGG_CONTRAST_Predictor, self).__init__()
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        self.use_bias = config.MODEL.ROI_RELATION_HEAD.FREQUENCY_BAIS
        ...
        self.context_layer = GSL4SGG_CONTRAST(config, in_channels, hidden_dim=self.hidden_dim)
...
def make_roi_relation_predictor(cfg, in_channels):
    func = registry.ROI_RELATION_PREDICTOR[cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR] # BGNN
    print(func)
