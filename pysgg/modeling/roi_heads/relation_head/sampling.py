...
class RelationSampling(object):
    def __init__(
        ...
        self.filter_low_score = filter_bg
    ...
    def detect_relsample(self, proposals, targets):
        ...
        rel_idx_pairs = []
        rel_idx_pairs_fully_connected = []
        rel_labels = []
        rel_labels_all = []
        rel_sym_binarys = []
        offset = 0
        gt_rel_label_fully_connected = []
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            device = proposal.bbox.device
            ...
            if target.has_field("relation_non_masked"):
                ...
                fg_gt_rel_pair_idx = gt_rel_idx[corrsp_gt_rel_idx[corrsp_gt_rel_idx >= 0]] # Proposal 중에 subject와 object가 모두 GT와 iou가 0.5이상인 GT
            ...
            if not self.use_gt_box:
                cand_matrix = torch.ones((len(proposal), len(proposal)), device=tgt_lab.device) - torch.eye(len(proposal), device=tgt_lab.device)
                idxs = torch.nonzero(cand_matrix).view(-1, 2)
                fg_relation_idx = torch.nonzero(corrsp_gt_rel_idx != -1)
                fg_obj_pair = img_rel_triplets[fg_relation_idx.view(-1)][:,:2]
                fg_obj = torch.unique(fg_obj_pair)
                if len(idxs) > 4096 and self.filter_low_score:
                    pairs_qualities = deepcopy(proposal.get_field('pred_scores'))
                    pairs_qualities[fg_obj] = 2.0
                    pairs_qualities = pairs_qualities[idxs[:, 0]] * pairs_qualities[idxs[:,1]]
                    select_idxs = torch.sort(pairs_qualities, descending=True)[-1][:4096]
                    idxs = idxs[select_idxs]
                gt_label = torch.zeros(len(idxs), device = tgt_lab.device)
                relative_gt_label_idx = [torch.nonzero((p[None,...] == idxs).all(-1)).item() for p in fg_obj_pair]
                for i in range(len(relative_gt_label_idx)):
                    relative_idx= relative_gt_label_idx[i]
                    rel_label = img_rel_triplets[i][2]
                    gt_label[relative_idx] = rel_label
                idxs += offset
                offset += len(proposal)
                rel_idx_pairs_fully_connected.append(idxs)
                gt_rel_label_fully_connected.append(gt_label)
            rel_idx_pairs.append(img_rel_triplets[:, :2])  # (num_rel, 2),  (sub_idx, obj_idx)
            rel_labels.append(img_rel_triplets[:, 2])  # (num_rel, )
            rel_sym_binarys.append(binary_rel) # Proposal 중에 subject와 object가 모두 GT와 iou가 0.5이상인 GT의 idx pair
        
        if len(rel_labels_all) == 0:
            rel_labels_all = rel_labels
        
        return proposals, rel_labels, rel_labels_all, rel_idx_pairs, rel_sym_binarys, gt_rel_label_fully_connected, rel_idx_pairs_fully_connected
    ...
    def motif_rel_fg_bg_sampling(self, device, tgt_rel_matrix, ious, is_match, rel_possibility, proposals_quality):
        ...
        for i in range(num_tgt_rels):
            # generate binary prp mask
            bi_match_head = torch.nonzero(binary_prp_head[i] > 0) # GT Subject가 Proposal과 Iou가 0.5d이상인 proposal index
            ...
            tgt_rel_lab = int(tgt_rel_labs[i]) # -1은 무엇?
            ...
            if num_match_head <= 0 or num_match_tail <= 0: # proposal에서 subject, object가 둘 다 gt와 0.5이상 iou를 갖으면 연결 제거
                continue
        ...
        # select fg relations
        if len(fg_rel_triplets) == 0: # Proposal 중에 Iou가 모두 0.5이상인 경우에 Label 부여
...
def make_roi_relation_samp_processor(cfg):
    samp_processor = RelationSampling(
        ...
        cfg.MODEL.ROI_RELATION_HEAD.GSL_MODULE.FILTER_BG,
    )
