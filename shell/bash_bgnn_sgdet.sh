...
export is_reporting=False
export reporting_thres=0.0
export report_criteria=max
export consideration_non_gt=False
export fill_criteria="object"
export reverse=False
export is_shift=True
export bg_filter=False
export divide_ratio=1.047 # 2.65, 1.5, 1.047

# zero
export zero_constant=0.327
export output_dir="checkpoints/${task}_BGNN_bilvl_${resampling}_refine_${use_obj_refine}_seton(${set_on})_gt(${apply_gt})_consideration_gt(${consideration_non_gt})_reporting(${is_reporting})_divide_ratio(${divide_ratio})_20percent"
# export output_dir="checkpoints/${task}_BGNN_ZERO_bilvl_${resampling}_refine_${use_obj_refine}_seton(${set_on})_gt(${apply_gt})_consideration_gt(${consideration_non_gt})_reporting(${is_reporting})_zero_constant(${zero_constant})_15percent"
export output_dir="checkpoints/${task}_BGNN_bilvl_${resampling}_refine_${use_obj_refine}"
