...
def train(
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.FIX_FEATURE:
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.RELATION_PROPOSAL_MODEL.SET_ON: # True
    ...
    fix_eval_modules(eval_modules) # Faster R-CNN을 모두 고정시켜놓는 함수.
    set_train_modules(train_modules) # 비어있음.

    if model.roi_heads.relation.rel_pn is not None: # None
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "IMPPredictor":
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.RELATION_CONFIDENCE_AWARE: # True
            slow_heads = [
                "roi_heads.relation.predictor.context_layer.relation_conf_aware_models",
            ]        

    except_weight_decay = []
    if cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == "BGNN_MODULE": # False
        if (
            cfg.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.RELATION_CONFIDENCE_AWARE # True
            and cfg.MODEL.ROI_RELATION_HEAD.BGNN_MODULE.RELNESS_MP_WEIGHTING_SCORE_RECALIBRATION_METHOD # learninable_scaling
            == "learnable_scaling"
        ):
            except_weight_decay = [
    ...
    optimizer = make_optimizer(
        ...
        model,
        logger,
        slow_heads=slow_heads, # "roi_heads.relation.predictor.context_layer.relation_conf_aware_models"
        slow_ratio=2.5,
    ...
    for iteration, (images, targets, _) in enumerate(train_data_loader, start_iter):
        ...
        if iteration % checkpoint_period == 0 and iteration and iteration > 15000:
            checkpointer.save("model_{:07d}".format(iteration), **arguments)
        if iteration == max_iter:
            checkpointer.save("model_final", **arguments)

        val_result_value = None  # used for scheduler updating
        if cfg.SOLVER.TO_VAL and iteration % cfg.SOLVER.VAL_PERIOD == 0 and iteration > 15000:
            if hasattr(model.roi_heads.relation.predictor.context_layer, 'gt2pred') :
                with open(cfg.OUTPUT_DIR+'/gt2pred_category_type.pkl', 'wb') as f:
                    pickle.dump(model.roi_heads.relation.predictor.context_layer.gt2pred, f)
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/relGSL4SGG_contrast_vg.yaml",
        metavar="FILE",
